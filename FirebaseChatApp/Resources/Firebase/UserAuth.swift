    ...
    public func loginUser(email:String, password:String, completion: @escaping ((Bool) -> Void)){
        Auth.auth().signIn(withEmail: email, password: password) { (authResult, error) in
            if let error = error as NSError? {
               ...
               // UserDefaults.standard.set(authResult?.user.uid, forKey: USER_DETAILS)
                completion(true)
            }
        }
    }
    
//    //MARK: - storeUserDetails
//    func saveUserData(userID:String, username:String, email:String, password:String, phone:String){
//        let data = [
//            "username": username,
//            "email": email,
//            "password": password,
//            "phone": phone
//        ]
//        database.collection(USER).addDocument(data: data) { error in
